...
pods_regs = {
    compile(r".*mqtt-broker-.+-.+\s+1/1\s+Running.*"): False,
...
uninstall_runtime_reg = {
    compile(r"release \"vehicleappruntime\" uninstalled"): False
}

reset_controlplane_reg = {
    compile(r"Successfully deleted cluster cluster!"): False
}
...
def reset_controlplane(reset_controlplane_reg: dict) -> bool:
    command_reset_controlplane_logs = check_output(BASE_COMMAND_RUNTIME.split(" ") + ["reset_controlplane"]).decode()
    return matches_any_regex(reset_controlplane_reg, command_reset_controlplane_logs)


def uninstall_runtime(uninstall_runtime_reg: dict) -> bool:
    command_uninstall_runtime_logs = check_output(BASE_COMMAND_RUNTIME.split(" ") + ["uninstall_runtime"]).decode()
    return matches_any_regex(uninstall_runtime_reg, command_uninstall_runtime_logs)


def run_command(command: str) -> bool:
    return (check_call(command, shell=True) == 0)


def test_scripts_run_successfully():
    assert run_command(f"{BASE_COMMAND_RUNTIME} configure_controlplane")
    assert check_cluster_status_and_dapr(kubectl_regs, dapr_regs)
    assert run_command(f"{BASE_COMMAND_RUNTIME} deploy_runtime")
    assert run_command(f"{BASE_COMMAND_DEPLOYMENT} build_vehicleapp")
    assert check_image_if_created(image_reg)
    assert run_command(f"{BASE_COMMAND_DEPLOYMENT} deploy_vehicleapp")
    assert check_pods(pods_regs)
    assert uninstall_runtime(uninstall_runtime_reg)
    assert reset_controlplane(reset_controlplane_reg)
